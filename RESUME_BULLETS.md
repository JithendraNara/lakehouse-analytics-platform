# Resume Bullets (Mapped to PROOF)

## Data Analyst / Analytics Engineer
- Built a medallion-style analytics pipeline (raw->staging->marts) that produced production-style KPI datasets for trend, conversion, and revenue analysis. (Proof: `L1`, `L3`)
- Implemented automated data quality controls with machine-readable validation reports to improve trust in reporting outputs. (Proof: `L2`)
- Delivered BI-ready exports powering daily KPI and experiment analysis workflows. (Proof: `L3`)

## Data Engineer
- Orchestrated end-to-end Python + SQL transformation workflows with reproducible artifact generation. (Proof: `L1`)
- Designed quality gates for duplicate keys, null timestamps, and metric-range constraints before downstream consumption. (Proof: `L2`)
- Created Snowflake SQL and Databricks/Spark parity artifacts to support warehouse migration and multi-platform execution. (Proof: `L4`)
